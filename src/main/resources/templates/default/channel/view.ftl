...
<script type="text/javascript">
    ...
    $(function(){
        $.each($(".markdown-body a"),function (index,item) {
            var _href = $(item).attr('href');
            var _fileName = $(item).html();

            if (_href.indexOf('.html') === -1)
            {
                $(item).click(function (event) {
                    var x=new XMLHttpRequest();
                    x.open("GET", _href, true);
                    x.responseType = 'blob';
                    x.onload=function(e){download(x.response, _fileName, ""); }
                    x.send();

                    return false;
                })
                // $(item).attr('href','/qny/down?sourceUrl=' + _href + '&fileName=' + _fileName);
